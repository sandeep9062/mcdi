  ...
  const filteredTestSeries = useMemo(() => {
    return testSeries.filter((ts) => {
      ...
      const matchesExamType =
        selectedExamType === "all" || ts.examType === selectedExamType;
      return matchesSearch && matchesExamType;
    });
  }, [searchQuery, selectedExamType, testSeries]);
          ...
          <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {examTypes.map((examType) => (
                  <Badge
                    key={examType}
                    variant={
                      selectedExamType === examType ? "default" : "outline"
                    }
                    className={`cursor-pointer ${
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {filteredTestSeries.map((series: TestSeries, index: number) => (
                  <motion.div
                    key={series.id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="p-6">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {series.whatIncluded.slice(0, 3).map((item: string, idx: number) => (
                            <div
                              key={idx}
                      ...
                      <Accordion type="single" collapsible className="mb-4">
                        ...
                        >
                          <AccordionTrigger className="text-sm font-medium">
                            ...
                            {series.sampleQuestions.length})
                          </AccordionTrigger>
                          <AccordionContent>
                            <div className="space-y-4 pt-2">
                              {series.sampleQuestions.map((q: { question: string; options: string[]; correctAnswer: number; explanation: string; }, qIdx: number) => (
                                <div
                                  key={qIdx}
              ...
              <div className="text-center py-20 bg-white rounded-xl shadow-md">
                ...
                <p className="text-gray-600">
                  Try adjusting your search or exam type filter
                </p>
              </div>
            )}
          </div>
